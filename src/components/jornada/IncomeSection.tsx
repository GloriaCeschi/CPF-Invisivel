...
export default function IncomeSection({ incomes, onRefresh }: IncomeSectionProps) {
  ...
  const handleDelete = async (id: string) => {
    const { error } = await supabase
      ...
      .delete()
      .eq("id", id)
      .eq("type", "income"); // garante que só remove rendas
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold font-display text-[hsl(218,26%,29%)]">
          Minhas Rendas
        </h2>
        <Button
          size="sm"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium">
                      {RECEIPT_LABELS[income.receipt_type || "pix"] || income.receipt_type}
                    </span>
                  </div>
                  {income.description && (
